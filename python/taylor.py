...
class TaylorVar(object):
    ...
    def __init__(self, value, derivatives=None):
        ...
        numel = np.prod(self.value.shape)
        assert ndim > 0 and ndim < 3, "Our c++ methods can't handle values with dimension < 1 or > 3, so we don't support them in Python either"
